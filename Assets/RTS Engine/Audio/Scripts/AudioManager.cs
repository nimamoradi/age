...
namespace RTSEngine
{
	...
	public class AudioManager : MonoBehaviour {
        ...
        public void Init (GameManager gameMgr)
        {
            ...
            Entity.EntityCreated += OnEntityCreated;
        ...
        {
            if(entity.AudioSourceComp)
                localAudioSources.Add(entity.AudioSourceComp); //add the entity's audio source component to the list.
        }
        ...
        {
            if(effectObj.AudioSourceComp)
                localAudioSources.Add(effectObj.AudioSourceComp); //add the effect object's audio source component to the list.
        }
        ...
        {
            volume = Mathf.Clamp01(volume); //volume's value can be only in [0.0, 1.0]
            ...
            if(SFXVolumeSlider)
                SFXVolumeSlider.value = volume; //update the music's volume UI slider as well
        }
        ...
        public void UpdateMusicVolume (float volume)
        {
            ...
            if(musicVolumeSlider)
                musicVolumeSlider.value = volume; //update the music's volume UI slider as well
        }
